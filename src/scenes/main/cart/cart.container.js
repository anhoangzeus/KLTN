...
const CartContainer = ({ navigation }) => {
    ...
    const GetAddress = () => {
        if (auth().currentUser) {
            database().ref('ListAddress').child(auth().currentUser.uid).once('value').then((snapshot) => {
                var item;
        ...
        if (auth().currentUser) {
            database().ref('Cart/' + auth().currentUser.uid).once('value').then((snapshot) => {
                ...
                snapshot.forEach((childSnapshot) => {
                    ...
                    total += Number(childSnapshot.val().Price * childSnapshot.val().Quantity);
                });
                setCartItem(items);
                setAmount(total);
            });
    ...
    const _checkGioHang = () => {
        if (amount !== 0 && hasAddress === true) {
            navigation.navigate('ItemsCart', { content: amount, listItem: CartItem, address: Address });
        } else {
            handleClosemodalPayment(true);
        }
    };
    const _tangSoLuong = (item) => {
        database().ref('Cart/' + auth().currentUser.uid + '/' + item.Id).update({
            Quantity: item.Quantity + 1,
        });
        CartItem.forEach(element => { if (element.Id === item.Id) { element.Quantity = item.Quantity + 1; } });
    ...
    const _xoaGioHang = () => {
        database().ref('Cart/' + auth().currentUser.uid).child(_idCanXoa).remove();
        ListenCart();
